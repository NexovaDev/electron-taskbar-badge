...
export default class BadgeGenerator {
	...
	generate(badgeContent) {
		if (!badgeContent) return;
		const opts = JSON.stringify(this.style);

		return this.win.webContents.executeJavaScript(`window.drawBadge = function ${this.drawBadge}; window.drawBadge(${typeof badgeContent === 'number' ? badgeContent : `'${badgeContent}'`}, ${opts});`);
	}

	drawBadge(badgeContent, style) {
		...
		img.draw = function() {
			...
			badgeContent = typeof badgeContent === 'number' ? this.badgeContent.toFixed(this.displayStyle.decimals) : badgeContent;

			const fontSize = Number(/[0-9.]+/.exec(this.ctx.font)[0]);

			if (typeof badgeNumber === 'number' && badgeContent > style.max) {
			...
			if (Object.keys(badgeTypes).some(type => type === badgeContent)) {
				...
				if (badgeContent === 'alert') {
					// Reduce y as asterisk is not in center
					y = y / 0.625;
					this.ctx.font = this.ctx.font.replace(fontSize, fontSize * 2);
				}
				if (badgeContent === 'attention') {
					this.ctx.font = this.ctx.font.replace(fontSize, fontSize * 1.25);
				}
				if (badgeContent === 'error') {
					this.ctx.font = this.ctx.font.replace(fontSize, fontSize * 1.75);
				}
				if (['available', 'away', 'busy', 'unavailable'].some(status => status === badgeContent)) {
					this.ctx.font = this.ctx.font.replace(fontSize, fontSize * 4);
				}
				this.ctx.fillText(badgeTypes[badgeContent], radius, y);
			}
			if (typeof badgeNumber === 'number' && badgeContent.length == 1) {
				this.ctx.fillText(badgeContent, radius, radius);
			}
			if (typeof badgeNumber === 'number' && badgeContent.length > 1) {
				fontWidth = this.ctx.measureText(badgeContent).width;
				...
				this.ctx.fillText(badgeContent, 0, 0);
